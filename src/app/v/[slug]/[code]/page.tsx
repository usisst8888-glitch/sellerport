...
export default async function StoreVideoCodePage({ params }: PageProps) {
  ...
  // tracking_links에서 해당 스토어의 영상번호 조회
  ...
  // 클릭 ID 생성
  const clickId = `sp_yt_${slug}_${normalizedCode}_${Date.now().toString(36)}`

  // 스마트스토어는 NT 파라미터, 그 외는 UTM 파라미터 사용
  if (isSmartStore) {
    // 네이버 스마트스토어: NT 파라미터만 사용 (URL 경로는 그대로 유지)
    targetUrl.searchParams.set('nt_source', 'youtube')
    targetUrl.searchParams.set('nt_medium', 'shorts')
    targetUrl.searchParams.set('nt_detail', `video_${normalizedCode.toLowerCase()}`)
    targetUrl.searchParams.set('nt_keyword', clickId)
  } else {
    // 그 외: UTM 파라미터 사용
    targetUrl.searchParams.set('utm_source', 'youtube')
    targetUrl.searchParams.set('utm_medium', 'shorts')
