...
export async function GET(
  ...
  try {
    ...
    // 1. tracking_links에서 해당 슬러그를 가진 틱톡 영상번호가 있는지 확인
    ...
    // 2. tiktok-{user_id} 형태의 슬러그인 경우 해당 유저가 존재하는지 확인
    if (slug.startsWith('tiktok-')) {
      const userIdPrefix = slug.replace('tiktok-', '')
    ...
    // 3. 틱톡 채널의 account_name이 slug와 일치하는지 확인
    ...
    console.error('TikTok store slug check error:', error)
    return NextResponse.json({
      success: false,
      error: '서버 오류가 발생했습니다.'
    }, { status: 500 })
