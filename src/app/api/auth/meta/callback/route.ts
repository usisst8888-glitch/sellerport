...
export async function GET(request: NextRequest) {
  try {
    ...
    // Instagram DM 연동은 이제 /api/auth/instagram/callback 에서 처리
    // (Instagram Login 방식으로 변경됨 - Facebook Page 불필요)
    if (stateType === 'instagram_dm' || stateType === 'instagram_login') {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}/${stateFrom}?error=use_instagram_login`
      )
    }
